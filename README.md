...
- `receiveCodecs` - codecs this peer prefers to **receive**, per kind, best first, applied with [`RTCRtpTransceiver.setCodecPreferences`](https://developer.mozilla.org/en-US/docs/Web/API/RTCRtpTransceiver/setCodecPreferences) where supported. Example: `{ video: ['video/AV1', 'video/VP9', 'video/VP8'], audio: ['audio/opus'] }`. This drives what the remote peer sends you. What you send is chosen by the remote peer's preference, or by [`RTCRtpEncodingParameters.codec`](https://developer.mozilla.org/en-US/docs/Web/API/RTCRtpEncodingParameters) on browsers that support it. Pass `{ prefer: [...], exclusive: true }` for a kind to negotiate only those codecs (plus rtx/red/ulpfec) in both directions, for instance on a browser that cannot choose its own send codec: the other codecs are left out of our offers and answers, and removed from the remote descriptions we apply, since a browser sends the codecs of the remote description whatever its own preferences. Codecs the browser cannot decode are ignored, and a preference matching none keeps the browser's default. The preference also applies to transceivers created by the remote offer, before the answer. It is applied again before every offer and answer, so changing `peer.receiveCodecs` and calling `peer.negotiate()` moves an established connection to the new codecs. `preferredCodecs` is a deprecated alias with the same meaning.
